...
def main():
    ...
    # Load Pretrained CNN Model without classification layer
    ...
    svm_model.fit(features_matrix, labels_array)
    
    predictions = svm_model.predict(features_matrix)
    acc = accuracy_score(labels_array, predictions)
    print(f"SVM Accuracy on Dataset: {(acc*100):.2f}%")
    ...
    joblib.dump(svm_model, 'cnn_svm_model.pkl')
    print("\nSaved SVM model to 'cnn_svm_model.pkl'")
